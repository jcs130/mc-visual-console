...
| 块 | 是什么 | 能否发布 | 说明 |
|---|---|---|---|
| **组件**（本仓根包 `mc-visual-console`） | 接缝协议与实现（`src/`）+ 自绘 2.5D 操作台（`client/`，含那个客户端页）+ 假 bot 与测试 | **可以** ✓ | **零派生资源、零 IP 牵连** —— 最干净的那一块。发布前需 `pnpm build:client` 并把构建产物 `client/app.js` 纳入 `files` |
